...
| #148 | fix: お問い合わせ確認画面の「戻って修正する」でフォームデータが消えるバグを修正 | fix/issue-148 |
| #149 | feat: リマインドバッチに送信件数のログ出力を追加 | feature/issue-149 |
| #151 | feat: 管理者ダッシュボードの実装（サマリー統計・直近予約一覧）+ 管理者レイアウトのサイドバー重なりバグ修正 | feature/issue-151 |
| #150 | feat: メールテンプレートにスタイルを適用 | feature/issue-150 |
| #161 | fix: メール送信処理をコントローラーからサービス層に移動 | fix/issue-161 |
...
| #198 | feat: プラン編集画面の画像管理改善（既存画像プレビュー・個別削除・追加アップロード） | feature/issue-198 |

| #200 | fix: 管理者画面のページネーションを Bootstrap 5 ビューに切り替える | fix/issue-200 |
| #202 | feat: 宿泊者プラン一覧のページネーションを整備する | feature/issue-202 |
| #212〜#215 | ci: デプロイジョブのタイムアウト耐性強化・bootstrap/cache パーミッション修正 | ci/issue-deploy-timeout 他 |
...
| Issue | 内容 | 優先度 |
|---|---|---|
| #211 | feat: 管理者画面絞り込み機能 | 高 |
| #201 | feat: 管理者画面ソート機能追加（#211 の後） | 中 |
| #206 | feat: プラン編集画面の画像削除UX改善（削除予定マーク表示） | 中 |
| #207 | chore: 本番サーバー構築手順をシェルスクリプト化する | 低 |
