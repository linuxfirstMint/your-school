...
| Issue | 内容 | 優先度 |
|---|---|---|
| #84 | フォームバリデーション表示の改善（必須マーク・日本語メッセージ） | 高 ← **次** |
| #72 | 空室カレンダー △（残りわずか）表示 | 高 |

#### フェーズ1：フロントエンド・UI の完成

| Issue | 内容 | 優先度 |
|---|---|---|
| #91 | Bootstrap5 のインストールおよび Vite への統合 | 高（#84・#72 完了後） |
| #92 | 共通レイアウトの UI 実装（ヘッダー・フッター・ナビゲーション） | 中（#91 完了後） |
| #93 | TOP ページの UI 実装（ヒーロー画像・コンセプト・プランへの導線） | 中（#92 完了後） |
| #94 | アクセス案内ページの UI 実装（Google Maps・交通手段・駐車場） | 中（#92 完了後） |
| #95 | 客室紹介ページの UI 実装（部屋タイプ別写真・設備・アメニティ） | 中（#92 完了後） |
...
| Issue | 内容 | 優先度 |
|---|---|---|
| #97 | メール送信基盤の構築（Mailpit 導入・Mailable クラス作成） | 低 |
| #98 | 各種システムメールの実装（予約完了・キャンセル完了） | 低（#97 完了後） |
| #99 | お問い合わせ機能の実装（宿泊者側フォーム・完了メール） | 低（#97 完了後） |
| #100 | お問い合わせ管理機能（管理者側一覧・詳細・ステータス管理） | 低（#99 完了後） |
| #101 | 予約状況リマインドバッチ（Task Scheduling による前日 10 時の自動メール） | 低（#97 完了後） |
